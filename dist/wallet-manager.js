...
async function waitForAccounts(p, { totalMs = 15000, stepMs = 400 } = {}) {
  ...
  disconnectWallet() {
	  ...
	  // === Binance W3W Provider 断开连接 ===
	  if (this.walletType === 'binance') {
		  try {
			  // Binance W3W Provider 支持 disconnect 方法
			  if (this.ethereum && typeof this.ethereum.disconnect === 'function') {
				  this.ethereum.disconnect();
				  console.log('[Binance] Provider disconnected');
			  }
		  } catch (e) {
			  console.warn('Error disconnecting Binance provider:', e);
		  }
	  }
